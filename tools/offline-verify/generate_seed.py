...
# BCrypt digest of the demo password below, generated with the `bcrypt` library
# and verified to round-trip. Cost 10 keeps seeding fast; real registrations use
# the application's configured cost of 12.
#
# These are throwaway credentials for local demo data only. The seed script is
# never loaded in the `prod` profile.
DEMO_PASSWORD = "DemoPassw0rd!"
DEMO_PASSWORD_HASH = "$2a$10$lAjc3OBNLZ3xwNB06udacO79th.CeTZoExFB8zWshfuatm7jXj6XW"
...
def build_activity() -> str:
    lines = [
        ...
        f"-- Every demo account shares the password: {DEMO_PASSWORD}",
        "-- The stored hash is a real BCrypt digest (cost 10), verified to round-trip.",
        "-- Throwaway local credentials; this script never loads under the `prod` profile.",
        "",
    ]
